...
export default function Dashboard() {
  const { data: stats, isError } = useQuery({
    ...
    retry: false // Don't keep retrying if backend is off
  })

  const statCards = [
    { label: 'Jobs Discovered', value: stats?.total_jobs || 0, icon: Briefcase, color: 'text-blue-600', bg: 'bg-blue-50' },
    { label: 'Applications Sent', value: stats?.total_applied || 0, icon: CheckCircle, color: 'text-indigo-600', bg: 'bg-indigo-50' },
    { label: 'Interviews', value: stats?.total_interviews || 0, icon: Clock, color: 'text-emerald-600', bg: 'bg-emerald-50' },
    { label: 'Avg Match Score', value: `${stats?.avg_match_score || 0}%`, icon: TrendingUp, color: 'text-orange-600', bg: 'bg-orange-50' },
  ]

  // Mock data for the chart to show the UI design even if backend is off
  const mockChartData = [
    { name: 'Mon', applied: 4 },
    { name: 'Tue', applied: 7 },
    { name: 'Wed', applied: 2 },
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto">
      ...
      {/* Backend Offline Warning (Only shows if Docker is off) */}
      {isError && (
        <div className="bg-red-50 border border-red-100 text-red-600 px-4 py-3 rounded-xl text-sm font-medium flex items-center">
          <span className="mr-2">⚠️</span> Backend is currently unreachable. Start Docker to see live data. Showing UI preview.
        </div>
      )}
      ...
      <div className="bg-gradient-to-r from-indigo-600 to-purple-600 rounded-3xl p-8 text-white shadow-lg shadow-indigo-200 flex justify-between items-center relative overflow-hidden">
        <div className="relative z-10">
          ...
          <button className="mt-6 bg-white text-indigo-600 px-6 py-2.5 rounded-full font-bold text-sm shadow-sm hover:shadow-md transition-shadow">
            Trigger Pipeline
          </button>
        </div>
        {/* Decorative circle */}
          ...
          <div className="space-y-4 flex-1">
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
              <div className="flex items-center gap-3">
                ...
                <span className="text-sm font-bold text-slate-700">Scraping Engine</span>
              </div>
              <span className="text-xs font-semibold text-slate-400">Idle (Next: 2h)</span>
            </div>
            <div className="flex items-center justify-between p-4 bg-slate-50 rounded-2xl">
              <div className="flex items-center gap-3">
                <div className="w-2 h-2 bg-green-500 rounded-full animate-pulse"></div>
